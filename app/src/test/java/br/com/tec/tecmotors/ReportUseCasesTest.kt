...
class ReportUseCasesTest {
    ...
    fun monthlyMetrics_and_summary_haveConsistentValues() {
        val vehicleId = 1L
        val fuels = listOf(
            FuelRecord(1, vehicleId, LocalDate.now().minusDays(25).toEpochDay(), 900.0, 20.0, 6.0),
            FuelRecord(2, vehicleId, LocalDate.now().minusDays(5).toEpochDay(), 1200.0, 25.0, 6.2)
        )
        val odometers = listOf(
            OdometerRecord(3, vehicleId, LocalDate.now().minusDays(1).toEpochDay(), 1400.0)
        )
        ...
        assertTrue(monthlyMetrics.any { it.totalCost > 0.0 })
